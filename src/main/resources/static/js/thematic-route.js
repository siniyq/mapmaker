...
// Удаляем этот экспорт, т.к. функция еще не определена
// window.buildOptimalThematicRoute = buildOptimalThematicRoute;

// Функция для добавления кнопки оптимизации маршрута
function addOptimizeRouteButton() {
    // Находим кнопку оптимизации маршрута и добавляем обработчик
    const optimizeButton = document.getElementById('optimize-route-button');
    if (optimizeButton) {
        optimizeButton.addEventListener('click', buildOptimalThematicRoute);
        console.log('Обработчик для кнопки оптимизации добавлен');
    } else {
        console.error('Кнопка оптимизации маршрута не найдена в DOM');
    }
}

// Вызываем функцию добавления кнопки сразу после загрузки скрипта
// Увеличиваем задержку до 500мс для уверенности, что DOM успеет загрузиться
setTimeout(addOptimizeRouteButton, 500);

// Функция для получения оптимального тематического маршрута
function buildOptimalThematicRoute() {
    clearMap();
    
    // Получаем тип POI и профиль маршрута
    const poiType = getCurrentPoiType();
    const profile = getCurrentProfile();
    
    // Получаем количество точек для маршрута
    const poiCount = getPoiCountValue();
    
    // Сначала получаем все доступные POI выбранного типа
    getPois(poiType)
        .then(pois => {
            if (pois.length < 3) {
                alert(`Недостаточно точек типа "${poiType}" для построения маршрута (нужно минимум 3)`);
                return;
            }
            
            // Отладочный вывод: проверяем наличие URL фотографий
            console.log("Получено точек интереса:", pois.length);
            console.log("Пример первой точки:", pois[0]);
            console.log("URL фотографии первой точки:", pois[0].photoUrl);
            
            // Сохраняем все доступные POI
            currentPois = pois;
            
            // Показываем индикатор загрузки
            showLoadingIndicator("Анализируем лучшие варианты маршрута...");
            
            // Фильтруем POI по рейтингу, если это возможно
            ...
            if (pois[0].hasOwnProperty('rating')) {
                ...
                // Берем только лучшие точки если их много
            ...
            const distanceMatrix = calculateDistanceMatrix(filteredPois);
            
            // Находим оптимальный маршрут с использованием "жадного" алгоритма
            const optimizedRoute = findOptimalRoute(filteredPois, distanceMatrix, poiCount);
            selectedPois = optimizedRoute;
            
            // Отмечаем, что маршрут оптимизирован
            routeOptimized = true;
            
            // Формируем строку координат для запроса API
            const pointsStr = selectedPois.map(poi => `${poi.latitude},${poi.longitude}`).join(';');
            
            // Показываем индикатор загрузки
            showLoadingIndicator("Строим маршрут через выбранные точки...");
            
            // Строим маршрут через API
        ...
        .catch(error => {
            ...
            alert(`Ошибка при получении точек интереса: ${error.message}`);
        });
}

// Экспортируем функцию в глобальную область видимости после ее определения
window.buildOptimalThematicRoute = buildOptimalThematicRoute;

// Расчет матрицы расстояний между точками
function calculateDistanceMatrix(pois) {
    const matrix = [];
...
// Алгоритм поиска оптимального маршрута с учетом рейтинга и расстояния
function findOptimalRoute(pois, distanceMatrix, maxPoints) {
    if (pois.length <= maxPoints) {
        return pois;
    }
    
    // Выбираем начальную точку с наивысшим рейтингом
    let selectedIndices = [];
    
    // Берем точку с наивысшим рейтингом как начальную
    selectedIndices.push(0);
    
    // Веса для расчета "выгоды" точки
    const RATING_WEIGHT = 0.7;
    const DISTANCE_WEIGHT = 0.3;
    
    // Выбираем остальные точки
    while (selectedIndices.length < maxPoints) {
        let bestScore = -1;
        let bestIndex = -1;
        
        // Для каждой потенциальной точки рассчитываем "выгоду"
        for (let i = 0; i < pois.length; i++) {
            ...
            for (const selectedIndex of selectedIndices) {
                const distance = distanceMatrix[selectedIndex][i];
                minDistance = Math.min(minDistance, distance);
            }
            
            // Нормализованное значение рейтинга (0-1)
            const ratingScore = pois[i].rating ? pois[i].rating / 5 : 0.5;
            
            // Нормализованное значение расстояния (обратное, ближе = лучше)
            // Максимальное расстояние примерно 5 км для города
            const distanceScore = 1 - Math.min(minDistance / 5, 1);
            
            // Итоговый скор с учетом весов
            const score = (ratingScore * RATING_WEIGHT) + (distanceScore * DISTANCE_WEIGHT);
    ...
    // Возвращаем выбранные точки в нужном порядке
    return selectedIndices.map(index => pois[index]);
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (optimizeButton) {
        optimizeButton.addEventListener('click', buildOptimalThematicRoute);
        console.log('Обработчик для кнопки оптимизации добавлен в DOMContentLoaded');
    }
